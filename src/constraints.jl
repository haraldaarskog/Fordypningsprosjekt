...
@constraint(model, [d = 1:D, r = 1:R], t_var[d, r] + sum(T_im[i, m] * w_var[i, u, d, r, m] for i = 1:I, u = 1:U, m in [1,2]) + sum(T_sd_Scan[s, d] * beta_var[s, d, r] for s = 1:S) + sum(T_i_MD[i] * gamma_var[i, d, r] for i = 1:I) >= K_dr[d, r] - Q_d )
...
@constraint(model, [r = 1:R], sum(t_var[d, r] for d = 1:D) + sum(T_im[i, m] * w_var[i, u, d, r, m] for i = 1:I, u = 1:U, d = 1:D, m in [1,2]) + sum(T_sd_Scan[s, d] * beta_var[s, d, r] for s = 1:S, d = 1:D) + sum(T_i_MD[i] * gamma_var[i, d, r] for i = 1:I, d = 1:D) == Q_w )
...
for i = 1:I, u = 1:U, d = 1:D
    if d + B_iu[i, u] <= 7
        ...
        @constraint(model, [i, u, d], sum(v_var[i, u, d, h, r] for r = 1:R, h = 1:(d - 1)) == 0 ) #Setter starten av uka =0
        @constraint(model, [i, u, d], sum(v_var[i, u, d, h, r] for r = 1:R, h = (d + B_iu[i, u] + 1):D if h <= D) == 0 )  #20
    else
        ...
        @constraint(model, [i, u, d], sum(v_var[i, u, d, h, r] for r = 1:R, h=((d + B_iu[i, u])%7) + 1:(d - 1)) == 0 )    #setter midten av uka =0
    end
end
...
@constraint(model, [i = 1:I, u = 1:U], sum(x_var[i, u, s, d] for s = 1:S, d = 1:D) == D_iuW[i, u] )
...
#@constraint(model, [r = 1:(R - 1)], sum(t_var[d, r] for d = 1:D) >= sum(t_var[d, r] for d = 1:D, r = (r + 1)) )

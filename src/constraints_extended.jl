...
@constraint(model, [d = 1:D, r = 1:R], h_var[d, r] == t_var[d, r] + sum(T_im[i, m] * a_var[i, u, h, d, r, m] for i = 1:I, u = 1:U, h = 1:H, m = 1:M) + sum(T_sd_Scan[s, d] * beta_var[s, d, r] for s = 1:S) + sum(T_i_MD[i] * gamma_var[i, d, r] for i = 1:I))
...
for i = 1:I, u = 1:U, d = 1:D
    if d + B_iu[i, u] <= 7
        ...
        @constraint(model, [i, u, d], sum(a_var[i, u, d, h, r, m] for m = 2, r = 1:R, h = 1:d - 1) == 0 )
        @constraint(model, [i, u, d], sum(a_var[i, u, d, h, r, m] for m = 2, r = 1:R, h = (d + B_iu[i,u] + 1:D) if h <= D) == 0 )
    else
        ...
        @constraint(model, [i, u, d], sum(a_var[i, u, d, h, r, m] for m = 2, r = 1:R, h = ((d + B_iu[i, u])%7) + 1:(d - 1)) == 0 )
    end
end

#Constraints connecting D_iuD to alpha
@constraint(model, [i = 1:I, u = 1:U], sum(alpha_var[i, u, d] for d = 1:D) >= D_iuD[i, u] )

#Constraints connecting alpha to x
...
#Constraints that ensure the radiologists get tasks connected to their specialization
@constraint(model, [i = 1:I, r = 1:R], sum(a_var[i, u, d, h, r, m] + gamma_var[i, d, r] for u = 1:U, d = 1:D, h = 1:H, m = 1:M) <= sum(K_dr[d, r] for d = 1:D) * C_ir[i, r] )

#Symmetry breaking constraints
#@constraint(model, [r = 1:(R - 1)], sum(a_var[i, u, d, h, r, m] for i = 1:I, u = 1:U, d = 1:D, h = 1:H, m = 1:M) >= sum(a_var[i, u, d, h, r, m] for i = 1:I, u = 1:U, d = 1:D, h = 1:H, r = (r + 1), m = 1:M) )
